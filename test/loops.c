...
void ompt_event_openmp_thread_begin_fn(ompt_thread_id_t id){
	if(id==5) log("Thread begin");
}

void ompt_event_initial_thread_begin_fn(ompt_thread_id_t id){
}

void ompt_event_openmp_thread_end_fn(ompt_thread_id_t id){
...
int ompt_initialize(ompt_function_lookup_t lookup, const char *runtime_version, int ompt_version) {
  ...
  REGISTER(ompt_event_openmp_thread_begin);
  REGISTER(ompt_event_initial_thread_begin);
  REGISTER(ompt_event_openmp_thread_end);
